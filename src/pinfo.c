...
int main(int argc, char *argv[]) {
	...
	if ((strlen(argv[0]) >= 3) || (use_manual))
		...
		if ((strstr(argv[0], "man") != NULL) || (use_manual)) {
			 ...
			 */
			for (i = 1; i < argc; i++) {
				strncat(filename, argv[i],
						sizeof(filename) - strlen(filename) - 2);
				strncat(filename, " ", 2);
			}
	...
	do {
		char *tmp;
		command_line_option =
			getopt_long(argc, argv, "hvmfrapcsdtn:lx", long_options, NULL);
		...
		case 'h':
			printf(
				_("Usage:\n"
				  ...
				  "-a, --apropos                         call apropos if "
				  "nothing found\n"
				  "-p, --plain-apropos                   call only apropos\n"
				  "-c, --cut-man-headers                 cut out repeated man "
				  "headers\n"
				  "-l, --long-manual-links               use long link names "
				  "in manuals\n"
				  "-s, --squeeze-manlines                cut empty lines from "
				  "manual pages\n"
				  "-d, --dont-handle-without-tag-table   don't display texinfo "
				  "pages without tag\n"
				  "                                      tables\n"
				  "-t, --force-manual-tag-table          force manual "
				  "detection of tag table\n"
				  "-x, --clear-at-exit                   clear screen at exit\n"
				  "    --node=nodename, --node nodename  jump directly to the "
				  "node nodename\n"
				  "    --rcfile=file, --rcfile file      use alternate "
				  "rcfile\n"),
				argv[0]);
			exit(0);
		case 'v':
	...
	if (id == NULL) {
		id = dirpage_lookup(&type, &message, &lines, filename,
							&pinfo_start_node);
	}
	...
	do {
		 ...
		 *
		 * the entire handling of file_name_force and work_return_value.file is
		 * a big hack, but it'll have to do for now, until the entire work loop
		 * thing is rewritten.
		 */
		if ((ForceManualTagTable == 0) &&
			...
			work_return_value =
				work(&message, &type, &lines, id, tag_table_pos);
		}
		if (work_return_value.node) {
			...
			} else /* file was specified */
			{
				...
				} else /* open new info file */
				{
					...
					/* try to lookup the name in dir file */
					if (id == NULL) {
						id = dirpage_lookup(&type, &message, &lines, tmp,
											&pinfo_start_node);
					}
					...
					if (id == NULL) {
						...
						mvhline(maxy - 1, 0, ' ', maxx);
						mvaddstr(maxy - 1, 0,
								 _("File not found. Press any key..."));
						move(0, 0);
						attrset(normal);
						getch();
						filenotfound = 1;
					...
					} else /* if we succeeded in opening new file */
					{
						...
						} else /* no tagtable found */
						{
							if (!DontHandleWithoutTagTable) {
								...
								mvhline(maxy - 1, 0, ' ', maxx);
								mvaddstr(maxy - 1, 0,
										 _("Tag table not found. Trying to "
										   "create alternate..."));
								create_tag_table(id);
								if (TagTableEntries < 1) {
									closeprogram();
									printf(_("This doesn't look like info "
											 "file...\n"));
									return 1;
								}
							} else
						...
						if (work_return_value.node[0] != 0) {
							int tmptagtablepos =
								gettagtablepos(work_return_value.node);
							if (tmptagtablepos != -1)
...
void checksu() {
	...
	if (!getegid() || !getgid()) {
		...
		if (!grwd) {
			if (verbose) {
				printf(_("Security warning: Unable to get GID of group called: "
						 "%s\n"),
					   safe_group);
				sleep(1);
			}
	...
	if (result == 0 && (!geteuid() || !getuid())) {
		...
		if (!pswd) {
			if (verbose) {
				printf(_("Security warning: Unable to get UID of user called: "
						 "%s\n"),
					   safe_user);
				sleep(1);
			}
