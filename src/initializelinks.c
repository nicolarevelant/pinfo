...
void sort_hyperlinks_from_current_line(long startlink, long endlink) {
 ...
 */
int exists_in_tag_table(char *item) {
...
void freelinks() /* frees space allocated previously by node-links */
{
...
char *findurlend(char *str) {
	char *end;
	char *allowedchars = "QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm1"
						 "234567890-_/~.%=|:@";
	end = str;
 ...
 */
/* TODO: fix possible string overflow (no bounds checking) */
char *finddot(char *str, int mynote) {
	char *ptr = str;
	char *end[4] = {0, 0, 0, 0};
	char *closest = 0;
	int i;
...
void initializelinks(char *line1, char *line2, int line) {
	...
	do {
		...
		if ((urlstart = findemailstart(urlend)) != NULL) {
			...
			if (!hyperobjectcount)
				...
				hyperobjects = xrealloc(
					hyperobjects, sizeof(HyperObject) * (hyperobjectcount + 1));
			}
			hyperobjects[hyperobjectcount].line = line;
	...
	if ((line1[0] == '*') && (line1[1] == ' ')) /* menu */
	{
		...
		if (tmp != NULL) {
			...
			if (line1[2] == '(') /* if cross-info link */
			{
				...
				if ((end != NULL) && (end < tmp)) {
					...
					strncpy(hyperobjects[hyperobjectcount].file, line1 + 3,
							FilenameLen);
					hyperobjects[hyperobjectcount].file[FilenameLen] = 0;
					strncpy(hyperobjects[hyperobjectcount].node, end + 1,
							NodenameLen);
				...
				int goodHit = 0;
				strcpy(hyperobjects[hyperobjectcount].file, "");
				strncpy(hyperobjects[hyperobjectcount].node, line1 + 2,
						NodenameLen);
				hyperobjects[hyperobjectcount].node[NodenameLen] = 0;
				hyperobjects[hyperobjectcount].type = 0;
				hyperobjects[hyperobjectcount].line = line;
		 ...
		 ******************************************************************************/
			...
			if (((start = strchr(tmp, '(')) != NULL) && (dot != NULL) &&
				...
				if ((start < dot) && (end < dot)) /* security mechanism ;) */
				{
					...
					if (!hyperobjectcount)
						...
						hyperobjects =
							xrealloc(hyperobjects, sizeof(HyperObject) *
													   (hyperobjectcount + 1));
					}
			...
			handle_no_file_menu_label: {
				...
				strcpy(hyperobjects[hyperobjectcount].file, "");
				strncpy(hyperobjects[hyperobjectcount].node, start,
						NodenameLen);
				hyperobjects[hyperobjectcount].node[NodenameLen] = 0;
				hyperobjects[hyperobjectcount].type = 1;
				hyperobjects[hyperobjectcount].line = line;
		...
		if (notestart < buf + strlen(line1)) {
			...
			if (tmp != NULL) {
					...
					if ((end != NULL) && (end < tmp)) {
						...
						if (notestart + 7 < buf + strlen(line1)) {
							...
							if (tmp < buf + strlen(line1)) {
								...
								hyperobjects[hyperobjectcount].breakpos =
									strlen(line1) -
									(long)(notestart + 7 - buf) + 1;
							}
						} else {
							hyperobjects[hyperobjectcount].line = line + 1;
							hyperobjects[hyperobjectcount].col = calculate_len(
								buf + strlen(line1), notestart + 7);
				...
				} else /* if not cross-info link */
				{
					...
					int goodHit = 0;
					strcpy(hyperobjects[hyperobjectcount].file, "");
					strncpy(hyperobjects[hyperobjectcount].node, notestart + 6,
							NodenameLen);
					hyperobjects[hyperobjectcount].node[NodenameLen] = 0;
					hyperobjects[hyperobjectcount].type = 2;
					hyperobjects[hyperobjectcount].nodelen =
					...
					if (exists_in_tag_table(
							hyperobjects[hyperobjectcount].node)) {
						hyperobjectcount++; /* yep, this was a good hit */
						goodHit = 1;
					}
					if (!goodHit && !hyperobjectcount) {
	...
	while ((urlstart = strstr(urlend, "http://")) != NULL) {
		...
		if (!hyperobjectcount)
			...
			hyperobjects = xrealloc(hyperobjects, sizeof(HyperObject) *
													  (hyperobjectcount + 1));
		}
		hyperobjects[hyperobjectcount].line = line;
		hyperobjects[hyperobjectcount].col = calculate_len(line1, urlstart);
