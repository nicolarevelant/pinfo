...
Suffixes suffixes[SuffixesNumber] = {{"", "cat"},
									 {".gz", "gzip -d -q -c"},
									 {".Z", "gzip -d -q -c"},
									 {".bz2", "bzip2 -d -c"}};
...
int qsort_cmp(const void *base, const void *compared) {
...
FILE *dirpage_lookup(char **type, char ***message, unsigned long *lines,
...
#define Lines (*lines)
	...
	for (unsigned long i = 1; i < Lines; i++) {
		if ((Message[i][0] == '*') && (Message[i][1] == ' ') &&
			...
			if (!((nameend - Message[i]) - 2 == filenamelen) && goodHit) {
				continue;
			}
...
#define Lines (*lines)
	...
	do {
		...
		/* TODO: Weirdness going on here; looks like off-by-one error as Buf[0]
		 * is always "\0" */
		...
		else /* we can be sure that at least 1 char was read! */
		{
			...
			while (i >= 0) {
			 ...
			 INFO_TAG); /* repeat until new node mark is found */
...
void load_tag_table(char **message, unsigned long lines) {
	...
	for (unsigned long i = 1; i < lines - is_indirect; i++) {
		...
		if (wsk1 < check) {
			 ...
			 */
			res = memcpy(tag_table[i - cut].nodename, wsk,
						 j = (size_t)(wsk1 - wsk));
			(*(res += j + 1)) = 0;
			(*wsk1) = INDIRECT_TAG;
			wsk1++;
...
/* TODO: seek_indirect() and seek_tag_table() are almost identical: remove
 * duplicate code */
int seek_indirect(FILE *id) {
					 ...
					 */
	{
		...
		if (fgets(type, 1024, id) == 0) {
			/* we're at the end of the file and haven't found any indirect refs.
			 * so bail out */
			if (type) {
				xfree(type);
				type = 0;
...
int seek_tag_table(FILE *id, int quiet) {
	...
	while (!finito) {
			...
			if (input == EOF) {
				if (!quiet) {
					if (!curses_open) {
						...
						mvhline(maxy - 1, 0, ' ', maxx);
						mvaddstr(maxy - 1, 0,
								 _("Warning: could not find tag table"));
						attrset(normal);
					}
				}
				if (type) {
		...
		while (fgetc(id) != '\n')
			;
		if (fgets(type, 1024, id) == NULL) {
			/* we're at the end of the file and haven't found a tag table. so
			 * bail out */
			if (type) {
				xfree(type);
				type = 0;
...
void buildcommand(char *dest, char *command, char *filename,
...
void builddircommand(char *dest, char *command, char *filename,
					 const char *tmpfilename) {
	strcpy(dest, command);
	strcat(dest, " ");
	strcat(dest, filename);
...
FILE *opendirfile(int number) {
	...
	char
		command[1128]; /* holds command to evaluate for decompression of file */
	int i, j;
	char *tmpfilename = NULL;
	size_t *fileendentries = xmalloc(
		infopathcount *
		sizeof(*fileendentries)); /* should really be off_t, but a signed type
	...
	for (i = 0; i < infopathcount; i++) /* go through all paths */
	{
		...
		if (!lang_found) {
			...
			for (j = 0; j < SuffixesNumber; j++) /* go through all suffixes */
			{
				...
				if ((id = fopen(buf, "r")) != NULL) {
					fclose(id);
					builddircommand(command, suffixes[j].command, buf,
									tmpfilename);
					xsystem(command);
					lstat(tmpfilename, &status);
					fileendentries[dircount] = status.st_size;
	...
	if (id) {
		...
		for (l = 0; l < filelen; l++) {
			...
			if (l + 1 == fileendentries[dircount]) {
				if (aswitch != 0)
					aswitch = 0;
				dircount++; /* the last dircount should fit to the end of
							   filelen */
			}
		}
		fputc(INFO_TAG, id);
...
#define BUF_LEN 1024
	...
	for (i = -2; i < infopathcount; i++) /* go through all paths */
	{
		if (i < 0) {
			...
			if (i == -2)
				snprintf(buf, BUF_LEN, "%s/%s", filenameprefix,
						 basename(filename));
			else
				snprintf(buf, BUF_LEN, "%s/%s.info", filenameprefix,
						 basename(filename));
		} else {
			/* build a filename */
...
void initpaths() {
	...
	/* TODO: cleanup all malloc calls (get rid of cast, use sizeof(varname)
...
void create_indirect_tag_table() {
	...
	for (unsigned i = 1; i <= IndirectEntries; i++) {
		...
		if (!id) {
			 ...
			 */
			char msg[1024];
			snprintf(msg, 1024, "%s '%s' (%s)", _("Can't open file"),
					 indirect[i].filename, _("press a key to continue"));
			attrset(bottomline);
			mvhline(maxy - 1, 0, ' ', maxx);
...
void create_tag_table(FILE *id) {
	...
	if (!tag_table)
		...
		tag_table =
			xrealloc(tag_table, (TagTableEntries + 2) * sizeof(TagTable));
	while (!feof(id)) {
									  ...
									  */
					{
						for (j = i + 2; j < buflen; j++) {
							if ((buf[j] == ',') || (buf[j] == '\n')) {
								...
								buf[j] = 0;
								buflen = j;
								strcpy(tag_table[TagTableEntries].nodename,
									   buf + i + 2);
	...
	if (indirect) {
		for (i = IndirectEntries; i >= 1; i--) {
			if (indirect[i].offset <= tag_table[tag_table_pos].offset) {
				long off = tag_table[tag_table_pos].offset -
						   indirect[i].offset + FirstNodeOffset - 4;
